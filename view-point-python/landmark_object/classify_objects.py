...
def classify_segments(image_dump_path, cl_dump, cluster_model, a_score, weather_info, geo_info, time_info, lmo_importance, img_id):
    ...
    lmo_id = cluster_model.predict(feature_list)

    for i in range(n_segments):
        # lmo_id = cluster_model.predict(feature_list[i])

        dump_cluster_info(lmo_id[i], cl_dump, a_score, weather_info, geo_info, time_info, saliency_list[i])

        # modify lmo matrix
        lmo_importance[img_id][lmo_id[i]] += 1
...
def process_dataset(model_path, dump_path):
    ...
    geo_minmax_info = dump_path + "geo_minmax.list"
    shutil.copy(geo_minmax_info, model_path)

    # matrix for lmo importance
    num_images = len(geo_info)
    # changed because using kmeans now
