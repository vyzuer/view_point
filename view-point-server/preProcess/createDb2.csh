...
while ($i < $num_of_clusters)
    ...
    pushd $i
        mkdir -p ImageDB
        mkdir -p InfoDB
        echo "name lat long views favorite exposure aperture ISO date time focal focal2 shutter-speed" > InfoDB/images.details2
    popd
    @ i++
end
...
while($j < $#clusterID)
    ...
    echo "$filename $lat $long $views $misc" >> ${label}/InfoDB/images.details2
